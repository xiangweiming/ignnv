%% Main function for check intersection of two polytopes
function res = intersectionHull(varargin)
%Syntax:
% res = intersectionHull('vert',V1,tess_V1,'vert',V2,tess_V2)
%
% Inputs:
%    'vert' - type: vertices (we only use vert in this project)
%    V1 - vertices
%    tess_V1 (optional, recomended)  tessellation of V1
%    'vert' - type: vertices
%    V2 - vertices
%    tess_V2 (optional, recomended)  tessellation of V2

% Outputs:
%    res - indicate existence of intersection, logical 1: exist, 0: note
%    
% Author:       Main code by Matt J's intersectionHull tool 
%               Edited by Weiming Xiang for this project.
% Written:      02/25/2019
% Last update:  02/25/2019

%Below is the document for Matt J's intersectionHull tool:
%
...
for i=1:L
    ...
    switch S(i).type
        ...
        case 'vert'
            if length(S(i).args) == 1
                [lcon{1:4}] = polyCompute.vert2lcon(S(i).args{:});
            else
                [lcon{1:4}] = polyCompute.vert2lcon(S(i).args{1},S(i).args{2});
            end
...
%------------------------------------
% use linear programming to check the feasibity to determine the if there
% exist intersection
...
if isempty(Aeq)
    ...
    constraint = [A*x <=b; Aeq*x == beq];
end
options = sdpsettings('verbose',0);
diagnostics = optimize(constraint,[], options);
...
%------------------------------------
% convert back to vertices to see if there exist vertices to determine the
% existence of intersection
% V = polyCompute.lcon2vert(A,b,Aeq,beq,TOL);
% res = isempty(V);
%
%I.vert=V';
